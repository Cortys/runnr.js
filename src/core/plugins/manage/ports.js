...
module.exports = {
	validate(ports) {
		...
		Object.keys(ports.in).forEach(name => ports.in[name] = constraints.validate(ports.in[name]));
		Object.keys(ports.out).forEach(name => ports.out[name] = constraints.validate(ports.out[name]));
